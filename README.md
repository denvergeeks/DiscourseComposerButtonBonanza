...
Here is the commit log for these changes: https://github.com/denvergeeks/DiscourseComposerButtonBonanza/commit/b72bf451b073f784531e912663292dbfd8aeffa8
...
a _bonanza_ of buttons.  Via the configuration settings, you can:
 * add and/or remove button definitions;
 * select which buttons to show, and when to show them;
 * change the ordering of buttons, and decide whether buttons should be rendered in the toolbar directly, or the toolbar's ⚙️ pop-up menu;
...
existing styles.  Its primary purpose is to help your users to discover
that the markup exists.  E.g., users who use a lot of footnotes will
probably just bang the `^[` and `]` directly on the keyboard; but the rest
of us will be happy to have an ✳️ button to remind us what to do.

A benefit of this approach is that if you decide the remove this theme
